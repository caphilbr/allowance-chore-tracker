...
allowanceRouter.patch("/", async (req, res) => {
  try {
    ...
    const existingAllowance = await Allowance.query().findOne({ userId: payload.userId });
    if (existingAllowance) {
      const updatedAllowance = await existingAllowance.$query().updateAndFetch(payload);
