...
export const Quiz = (props) => {
    ...
    if (responseStatus == null) {
      ...
      if (answer == correctAnswer) {
        try {
          ...
          console.log("response from PostQuizResopnse", response)
          if (!response.ok) {
            ...
            const updateQuizDate = response.user.body.quizDate
            props.addTranscation(newTransaction)
            // props.setCurrentUser(updateUser)
            props.changeQuizDate(updateQuizDate)
          }
        } catch(error) {
  ...
  let answerList = null
  if (correctAnswer != "") {
    const answerOptions = [...questionObject.incorrect_answers, questionObject.correct_answer]
    answerList = answerOptions.map(answerChoice => {
      return (
        <div key={answerChoice} className="cell small-5">
          <span className="quiz-answers" onClick={handleAnswerChoice} value={answerChoice}>
            {parse(answerChoice)}
          </span>
        </div>
      )
  ...
  const questionDisplay = (
    <>
      ...
      <span className="button-styling" onClick={handleClose}>Close</span>
    </>
  )
